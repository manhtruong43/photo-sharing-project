...
mongoose.connect('mongodb://blogtest1:qR5TyA8oj9ZF7xsw@ac-yxezx0n-shard-00-00.tkzyyui.mongodb.net:27017/cs142project7?authSource=admin&replicaSet=atlas-ebpyi7-shard-0&ssl=true&appName=Cluster0', { useNewUrlParser: true, useUnifiedTopology: true });
...
app.use(function(request, response, next) {
    var publicPaths = ['/admin/login', '/admin/logout', '/user'];
...
app.get('/user/list', function (request, response) {
    User.find({}, function(err, users) {
        ...
        Photo.find({}, function(photoErr, photos) {
            if (photoErr) {
                console.error('Doing /user/list photo count error: ', photoErr);
            ...
            const userList = users.map(user => {
                ...
                const photoCount = photos.filter(p => p.user_id && p.user_id.toString() === userIdStr).length;
                // Count comments authored by this user
                let commentCount = 0;
                photos.forEach(p => {
                    if (p.comments) {
                        p.comments.forEach(c => {
                            if (c.user_id && c.user_id.toString() === userIdStr) {
                                commentCount++;
                            }
                        });
                    }
...
/*
 * URL /commentsOfUser/:id - Return all comments authored by User (id)
 */
app.get('/commentsOfUser/:id', function (request, response) {
    ...
    Photo.find({}, function(err, photos) {
        if (err) {
            console.error('Doing /commentsOfUser/:id error: ', err);
            response.status(500).send(JSON.stringify(err));
            return;
        }
        ...
        userComments.sort((a, b) => new Date(a.date_time) - new Date(b.date_time));
